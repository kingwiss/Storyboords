document.addEventListener('DOMContentLoaded', () => {
    ...
    const audiobookTitle = document.getElementById('audiobook-title');
    ...
    const aiImagesGallery = document.getElementById('ai-images-gallery');
...
function loadArticleIntoViewer(audiobook) {
    ...
    const audiobookTitle = document.getElementById('audiobook-title');
    console.log('audiobookTitle element:', audiobookTitle);
    if (audiobookTitle && audiobook.title) {
        audiobookTitle.textContent = audiobook.title;
        console.log('Set title to:', audiobook.title);
    }
    
    // Set the global fullArticleText for audio functionality
    fullArticleText = audiobook.fullText || '';
    
    // Update article text with proper word highlighting setup
    const articleText = document.getElementById('article-text');
    if (articleText && audiobook.fullText) {
        // Use the displayArticleWithWordHighlighting function for proper word setup
        displayArticleWithWordHighlighting(audiobook.fullText);
    }
    
    // Update summary
    const summaryText = document.getElementById('summary-text');
    if (summaryText && audiobook.summary) {
        summaryText.textContent = audiobook.summary;
    }
    
    // Update key points
    const keyPointsList = document.getElementById('key-points-list');
    if (keyPointsList && audiobook.keyPoints) {
        keyPointsList.innerHTML = '';
        const keyPoints = Array.isArray(audiobook.keyPoints) ? audiobook.keyPoints : 
            (typeof audiobook.keyPoints === 'string' ? audiobook.keyPoints.split('\n').filter(point => point.trim()) : []);
    ...
    // Update AI images if available
    const imagesGallery = document.getElementById('ai-images-gallery');
    if (imagesGallery && audiobook.imageUrls && audiobook.imageUrls.length > 0) {
        imagesGallery.innerHTML = '';
        audiobook.imageUrls.forEach(imageUrl => {
        ...
        imagesGallery.innerHTML = '<p>No images available for this article.</p>';
    }
...
function applyTheme(theme) {
    ...
    // Apply new theme using both methods for compatibility
    if (theme === 'dark') {
...
function escapeHtml(text) {
    ...
    function restoreArticleState() {
        try {
            ...
            if (state.fullText) {
                ...
                // Show the audiobook container
                if (audiobookContainer) {
                    console.log('Showing audiobook container');
                    audiobookContainer.style.display = 'block';
                } else {
                    console.log('audiobookContainer not found');
                }
                ...
                // Set global state
                fullArticleText = state.fullText;
                ...
                console.log('fullArticleText length:', fullArticleText.length);
    ...
    // Show/hide floating active article button
    ...
    function hideFloatingButton() {
        const activeArticleBtn = document.getElementById('active-article-btn');
        if (activeArticleBtn) {
            activeArticleBtn.style.display = 'none';
        }
    }
    
    // Check if there's an active article and show floating button
    function checkAndShowFloatingButton() {
        try {
            ...
            if (savedState) {
                ...
                // Check if state is not too old and has article content
                if (now - state.timestamp <= maxAge && state.fullText) {
                    // Check if TTS is active or if we're not on the article-view page
                    const isTTSActive = speechSynthesis.speaking || speechSynthesis.pending;
                    const isOnArticleView = window.location.pathname.includes('article-view.html');
                    
                    if (isTTSActive || !isOnArticleView) {
                        showFloatingButton();
                        
                        // Update button text based on TTS status
                        const statusSpan = document.querySelector('#active-article-btn .article-status');
                        if (statusSpan) {
    ...
    function populateVoiceSelect() {
        ...
        // voiceControl IS the select element, not a container
        const voiceSelect = voiceControl;
        ...
        // Add voice options using index-based values for consistency
        englishVoices.forEach((voice, index) => {
            const option = document.createElement('option');
            option.value = index;
            option.textContent = `${voice.name} (${voice.lang})`;
        ...
        // Set default selection to first high-quality voice if available
    ...
    generateBtn.addEventListener('click', async () => {
        ...
        try {
            ...
            // Set title and full article text
            try {
                audiobookTitle.textContent = data.title;
                fullArticleText = data.fullText;
                totalWords = fullArticleText.split(' ').length;
                
                // Display full article with word highlighting
                displayArticleWithWordHighlighting(data.fullText);
                
                // Set summary and key points
                ...
                aiImagesGallery.innerHTML = '';
                if (data.imageUrls && data.imageUrls.length > 0) {
                    data.imageUrls.forEach((imageUrl, index) => {
                        const imageWrapper = document.createElement('div');
                        imageWrapper.className = 'ai-image-wrapper';
                        ...
                        aiImagesGallery.appendChild(imageWrapper);
                    });
                }
    ...
    function displayArticleWithWordHighlighting(text) {
        // Get the article text element dynamically
        const articleText = document.getElementById('article-text');
        if (!articleText) {
        ...
        const htmlContent = paragraphs.map(paragraph => {
            ...
            const wrappedWords = words.map(word => {
                const span = `<span class="word" data-word-index="${wordIndex}">${word}</span>`;
                wordIndex++;
                return span;
            }).join(' ');
        ...
        wordElements = Array.from(articleText.querySelectorAll('.word'));
    ...
    function highlightWord(index) {
        ...
        if (index >= 0 && index < wordElements.length) {
            ...
            // Auto-scroll disabled - user can manually scroll as needed
    ...
    function showScrollButton() {
        ...
        scrollToCurrentBtn.classList.remove('hidden');
    }
    ...
    function speakText(text, startWordIndex = 0) {
        if (!text || wordElements.length === 0) return;
        
        currentWordIndex = startWordIndex;
        const wordsToSpeak = wordElements.slice(startWordIndex).map(el => el.textContent).join(' ');
        
        currentUtterance = new SpeechSynthesisUtterance(wordsToSpeak);
        ...
        // Use boundary events for precise word tracking
         currentUtterance.onboundary = (event) => {
             if (event.name === 'word' && isPlaying) {
                 const wordIndex = startWordIndex + getWordIndexFromCharIndex(event.charIndex, wordsToSpeak);
                 // Highlight the next word to compensate for boundary event timing
                 const highlightIndex = wordIndex + 1;
                 if (highlightIndex < totalWords) {
                     currentWordIndex = highlightIndex;
        ...
        currentUtterance.onstart = () => {
            isPlaying = true;
            // Don't change button text here as it's handled in click event
    ...
    playPauseBtn.addEventListener('click', (e) => {
        ...
        if (!fullArticleText) {
            alert('Please generate an audiobook first.');
            return;
        }
        ...
        if (isPlaying) {
            ...
            // Start speech with a small delay to ensure clean state
            setTimeout(() => {
                speakText(fullArticleText, currentWordIndex);
                playPauseBtn.disabled = false;
                isProcessingClick = false;
    ...
    stopBtn.addEventListener('click', (e) => {
        ...
        // Clear article state when stopping
        clearArticleState();
    ...
    progressBar.addEventListener('click', (e) => {
        ...
        currentWordIndex = Math.floor(clickPosition * totalWords);
    ...
    // Voice control event listener
    if (voiceControl) {
        voiceControl.addEventListener('change', handleVoiceChange);
    }
    
    // Add debug button event listener
    const debugRestoreBtn = document.getElementById('debug-restore-btn');
    if (debugRestoreBtn) {
        debugRestoreBtn.addEventListener('click', () => {
            console.log('=== DEBUGGING: Manual restore button clicked ===');
            restoreArticleState();
        });
    }
    ...
    // Restore article state on page load (with delay to ensure DOM is ready)
    setTimeout(() => {
        console.log('=== DEBUGGING: Calling restoreArticleState after DOM setup ===');
        restoreArticleState();
    }, 100);
...
function checkForArticleData() {
    console.log('=== DEBUG: checkForArticleData called');
    const articleData = sessionStorage.getItem('viewArticleData');
    console.log('=== DEBUG: Raw sessionStorage data:', articleData ? 'EXISTS' : 'NOT FOUND');
    
    if (articleData) {
        console.log('=== DEBUG: Raw sessionStorage data length:', articleData.length);
        console.log('=== DEBUG: First 200 chars of sessionStorage data:', articleData.substring(0, 200));
        
        try {
            const article = JSON.parse(articleData);
            console.log('=== DEBUG: Successfully parsed article data');
            console.log('=== DEBUG: Article title:', article.title);
            console.log('=== DEBUG: Article content length:', article.content ? article.content.length : 'NO CONTENT');
            console.log('=== DEBUG: Full parsed article data:', article);
            
            sessionStorage.removeItem('viewArticleData');
            console.log('=== DEBUG: Removed viewArticleData from sessionStorage');
            
            // Display the article directly without regeneration
            console.log('=== DEBUG: Calling displayExistingArticle');
            displayExistingArticle(article);
        } catch (error) {
            console.error('=== DEBUG: Error processing article data:', error);
            console.error('=== DEBUG: Error stack:', error.stack);
        }
    } else {
        console.log('=== DEBUG: No viewArticleData found in sessionStorage');
    }
}

// Display existing article from My Articles
function displayExistingArticle(article) {
    console.log('=== DEBUG: displayExistingArticle called with:', article);
    console.log('=== DEBUG: Article content length:', article.content ? article.content.length : 'No content');
    console.log('=== DEBUG: Article keyPoints:', article.keyPoints);
    console.log('=== DEBUG: Article imageUrls:', article.imageUrls);
    ...
    console.log('=== DEBUG: DOM elements found:', {
        articleUrlInput: !!articleUrlInput,
        audiobookContainer: !!audiobookContainer,
        loadingContainer: !!loadingContainer,
        audiobookTitle: !!audiobookTitle,
        articleText: !!articleText,
        summaryText: !!summaryText,
        keyPointsList: !!keyPointsList,
        aiImagesGallery: !!aiImagesGallery
    });
    
    // Update the article URL input
    console.log('=== DEBUG: Setting article URL input');
    if (articleUrlInput) {
        articleUrlInput.value = article.url || '';
        console.log('=== DEBUG: Article URL set to:', article.url);
    } else {
        console.log('=== DEBUG: articleUrlInput not found');
    }
    
    // Show the audiobook container
    console.log('=== DEBUG: Showing audiobook container');
    if (audiobookContainer) {
        audiobookContainer.style.display = 'block';
        console.log('=== DEBUG: Audiobook container displayed');
    } else {
        console.log('=== DEBUG: audiobookContainer not found');
    }
    
    // Hide loading container
    console.log('=== DEBUG: Hiding loading container');
    if (loadingContainer) {
        loadingContainer.style.display = 'none';
        console.log('=== DEBUG: Loading container hidden');
    } else {
        console.log('=== DEBUG: loadingContainer not found');
    }
    
    // Set the title
    console.log('=== DEBUG: Setting article title');
    if (audiobookTitle) {
        audiobookTitle.textContent = article.title || 'Untitled Article';
        console.log('=== DEBUG: Title set to:', article.title);
    } else {
        console.log('=== DEBUG: audiobookTitle not found');
    }
    
    // Display article content with word highlighting
    console.log('=== DEBUG: Setting article content');
    if (article.content && articleText) {
        console.log('=== DEBUG: Calling displayArticleWithWordHighlighting with content length:', article.content.length);
        displayArticleWithWordHighlighting(article.content);
        console.log('=== DEBUG: Article content displayed');
    } else {
        console.log('=== DEBUG: No content or articleText element missing. Content exists:', !!article.content, 'Element exists:', !!articleText);
    }
    
    // Display summary
    console.log('=== DEBUG: Setting summary');
    if (article.summary && summaryText) {
        summaryText.textContent = article.summary;
        console.log('=== DEBUG: Summary set:', article.summary.substring(0, 100) + '...');
    } else {
        console.log('=== DEBUG: No summary or summaryText element missing. Summary exists:', !!article.summary, 'Element exists:', !!summaryText);
    }
    
    // Display key points
    console.log('=== DEBUG: Setting key points');
    if (article.keyPoints && keyPointsList) {
        keyPointsList.innerHTML = article.keyPoints
            .map(point => `<li>${escapeHtml(point)}</li>`)
            .join('');
        console.log('=== DEBUG: Key points set, count:', article.keyPoints.length);
    } else {
        console.log('=== DEBUG: No keyPoints or keyPointsList element missing. KeyPoints exists:', !!article.keyPoints, 'Element exists:', !!keyPointsList);
    }
    
    // Display AI images if available
    console.log('=== DEBUG: Setting AI images');
    if (article.imageUrls && article.imageUrls.length > 0 && aiImagesGallery) {
        aiImagesGallery.innerHTML = article.imageUrls
            .map(url => `<img src="${url}" alt="AI Generated Image" class="ai-image">`)
            .join('');
        aiImagesGallery.style.display = 'block';
        console.log('=== DEBUG: AI images set, count:', article.imageUrls.length);
    } else {
        console.log('=== DEBUG: No images or aiImagesGallery element missing. Images exist:', !!article.imageUrls, 'Images count:', article.imageUrls ? article.imageUrls.length : 0, 'Element exists:', !!aiImagesGallery);
    }
    
    // Reset audio player state
    console.log('=== DEBUG: Resetting audio player');
    resetAudioPlayer();
    console.log('=== DEBUG: displayExistingArticle completed successfully');
}
