document.addEventListener('DOMContentLoaded', function() {
    ...
    function initializeTextToSpeech(articleText) {
        ...
        const savePlaybackState = () => {
            if (ttsState.isPlaying || speechSynthesis.speaking) {
        ...
        // Priority: persistent audio manager state > old ttsState
        
        // First check persistent audio manager state
        if (window.persistentAudio && window.persistentAudio.articleId === currentArticle.id) {
    ...
    function showScrollButton() {
        ...
        if (!scrollBtn) {
            ...
            scrollBtn.className = 'scroll-to-current';
    ...
    setTimeout(async () => {
        ...
        const isPlaying = window.musicPlayer.isCurrentlyPlaying;
        console.log('=== DEBUG: article-view.js: Music player status:', { isPlaying });
    ...
    if (musicToggleBtn) {
        ...
        musicToggleBtn.addEventListener('click', async (e) => {
            ...
            isToggling = true;
            const originalState = window.musicPlayer.isCurrentlyPlaying;
            ...
            try {
                ...
                if (originalState) {
                    console.log('=== DEBUG: Stopping music via toggle button ===');
                    ...
                    setTimeout(() => {
                        if (!window.musicPlayer.isCurrentlyPlaying) {
                            ...
                            console.log('=== DEBUG: Music stopped successfully ===');
                        } else {
                            console.warn('=== DEBUG: Music stop may have failed ===');
                        }
                    }, 200);
                } else {
                    console.log('=== DEBUG: Starting music via toggle button ===');
                    await window.musicPlayer.start();
                    ...
                    setTimeout(() => {
                        if (window.musicPlayer.isCurrentlyPlaying) {
                            musicToggleBtn.classList.add('active');
                            console.log('=== DEBUG: Music started successfully ===');
                        } else {
                            console.warn('=== DEBUG: Music start may have failed ===');
                        }
                    }, 200);
                }
            } catch (error) {
