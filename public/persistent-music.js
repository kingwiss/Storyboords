...
class PersistentMusicManager {
    ...
    init() {
        ...
        // Check if music is already playing in another tab/window
        const existingState = this.getSharedState();
        const isCurrentlyPlaying = existingState && this.isRecentState(existingState) && existingState.isPlaying;
        
        if (isCurrentlyPlaying) {
            console.log('=== DEBUG: Music already playing in another context, syncing... ===');
            this.syncWithExistingMusic();
        } else {
            console.log('=== DEBUG: Initializing new player, which will auto-start ===');
    ...
    syncWithExistingMusic() {
        ...
        this.musicPlayer = {
            ...
            stop: () => {
                const currentState = this.getSharedState();
                if (this.musicPlayer.realPlayer) {
                    this.musicPlayer.realPlayer.stop();
                }
    ...
    isRecentState(state) {
        ...
        return timeDiff < 60000; // 60 seconds - allow more time for page navigation
    }
    ...
    stop() {
        ...
        this.musicPlayer.stop();
        console.log('=== DEBUG: Persistent music stopped ===');
    }
