...
Provide the output in the following JSON format:
{
  ...
  "imagePrompts": ["detailed specific prompt 1 with concrete visual elements from article", "detailed specific prompt 2 with concrete visual elements from article", "detailed specific prompt 3 with concrete visual elements from article"]
        ...
        // Enhanced image generation using Pollinations.ai with improved error handling
        const generateImage = async (prompt, retryCount = 0) => {
            ...
            try {
                ...
                // Try different models/parameters based on retry count
                const models = ['flux', 'turbo', 'deliberate'];
                const currentModel = models[retryCount % models.length];
                
                // Add parameters for higher quality and more relevant images
                const imageUrl = `https://image.pollinations.ai/prompt/${encodedPrompt}?width=800&height=600&nologo=true&enhance=true&model=${currentModel}`;
                
                console.log(`Generating image with Pollinations.ai (attempt ${retryCount + 1}):`, imageUrl);
                ...
                console.error(`Error generating image with Pollinations.ai (attempt ${retryCount + 1}):`, {
                ...
                console.error(`Failed to generate image after ${maxRetries + 1} attempts:`, prompt.substring(0, 100));
                return null;
            }
        };
